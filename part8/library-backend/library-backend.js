...
const resolvers = {
  Query: {
    ...
    allBooks: (root, args) => {
      ...
      let res = books;
      if (author) res = res.filter(authorFilter);
      if (genre) res = res.filter(genreFilter);
      return res;
    },
  ...
  Mutation: {
    addBook: (root, args) => {
      ...
      if (books.some((book) => does(book).match({ title, author, published })))
      ...
      if (
        !authors.some((authorObject) =>
          does(authorObject).match({ name: author })
        )
      )
